...
class TupleForm(FlaskForm):
    ...
    best_item = RadioField(
        "Label",
        coerce=int,
        validators=[
            InputRequired(message="Choose one item!"),
            NotEqualTo("worst_item", message="Two questions for this tuple require 2 different answers"),
        ],
    )
    worst_item = RadioField(
        "Label",
        coerce=int,
        validators=[
            InputRequired(message="Choose one item!"),
            NotEqualTo("best_item", message="Two questions for this tuple require 2 different answers"),
        ],
    )
...
class AnnoCheckinForm(FlaskForm):
    ...
    keyword = PasswordField(
        "Keyword as annotator",
        validators=[
            InputRequired(message=""),
            InputValid(model=Annotator, field=Annotator.keyword, message="Invalid keyword!"),
        ],
    )
    name = StringField("Specify your name", validators=[InputRequired(message="")])
    ...
    def validate(self, extra_validators=None):
        ...
        if (
            Annotator.query.filter(
                Annotator.keyword == self.keyword.data, Annotator.name.isnot(None), Annotator.name == self.name.data
            ).first()
            or Annotator.query.filter(Annotator.keyword == self.keyword.data, Annotator.name == None).first()
        ):
            return True
        self.name.errors.append(
            "This name is not used for this keyword! Have you logged in with this keyword before? If not, someone has already used it!"
        )
        return False
