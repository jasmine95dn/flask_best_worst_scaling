...
@pytest.mark.parametrize(
    ...
    [
        "/user/jung",
        "/user/jung/test",
        "/user/jung/test/keywords.txt",
        "/user/jung/test/report.txt",
        "/user/jung/test/scores.txt",
    ],
)
...
def test_login_required_user(test_client, route1, route2):
    ...
    # 2.
    assert response.headers["Location"].startswith("/user/login?next=%2Fuser%2Fjung")

    response = test_client.get(route2)
    # 1.
    assert response.status_code == 302
    # 2.
    assert response.headers["Location"].startswith("/user/login?next=%2Fuser%2Fupload-project")
...
def test_login_required_annotator(test_client, route):
    ...
    assert response.status_code == 302
    # 2.
    assert response.headers["Location"].startswith("/annotator?next=%2Fannotator%2Ftest")
