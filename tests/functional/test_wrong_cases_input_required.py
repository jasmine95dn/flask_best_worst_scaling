...
def test_no_password_login(test_client, init_database):
    ...
    ### a.
    response1 = test_client.post(
        "/user/login", data=dict(username="jung", password=None, remember=True), follow_redirects=True
    )
    # 1.
    assert response1.status_code == 200
    # 2.
    ...
    ### b.
    response2 = test_client.post(
        "/user/login", data=dict(username="jung", password="", remember=True), follow_redirects=True
    )
    # 1.
    assert response2.status_code == 200
    # 2.
    ...
    ### c.
    response3 = test_client.post("/user/login", data=dict(username="jung", remember=True), follow_redirects=True)
    # 1.
    assert response3.status_code == 200
    # 2.
