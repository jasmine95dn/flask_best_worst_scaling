...
def test_valid_annotator(test_client, init_database):
    ...
    """
    response = test_client.post("/annotator", data=dict(keyword="ax7832ljf", name="jung"), follow_redirects=True)
    # 1.
	...
	THEN check if is redirected to the right route:
		...
		it has the buttons to all batches for annotation
	"""
    response = test_client.post("/annotator", data=dict(keyword="kjd8f9s879", name="sanaz"), follow_redirects=True)
    # 1.
    assert response.status_code == 200
    # 2.
    assert b"TEST" in response.data
...
def test_invalid_keyword(test_client, init_database):
    ...
    """
    response = test_client.post("/annotator", data=dict(keyword="kjd8f9s87", name="sanaz"), follow_redirects=True)
    # 1.
    assert response.status_code == 200
    # 2.
...
def test_invalid_name(test_client, init_database):
    ...
    """
    response = test_client.post("/annotator", data=dict(keyword="kjd8f9s879", name="maryna"), follow_redirects=True)
    # 1.
    assert response.status_code == 200
    # 2.
    assert b"Annotator - Main page" in response.data
    # 3.
    assert (
        b"""This name is not used for this keyword! Have you logged in with this keyword before? If not, someone has already used it!"""
        in response.data
    )
