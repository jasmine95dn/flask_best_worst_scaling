...
def test_upload_file_no_required_extensions(test_client, init_database):
    ...
    response = test_client.post(
        "/user/upload-project",
        data=dict(
            upload=[open(os.path.join(basedir, "examples/first_10_characters_examples"), "rb")],
            name="first 10 characters again",
            description="this is an another test about the first 10 characters",
            anno_number=5,
            best_def="the best character",
            worst_def="the worst character",
        ),
...
def test_upload_empty_files(test_client, init_database):
    ...
    response = test_client.post(
        "/user/upload-project",
        data=dict(
            upload=[open(os.path.join(basedir, "examples/empty_example.txt"), "rb")],
            name="first 10 characters again",
            description="this is an another test about the first 10 characters",
            anno_number=5,
            best_def="the best character",
...
def test_upload_few_items(test_client, init_database):
    ...
    response = test_client.post(
        "/user/upload-project",
        data=dict(
            upload=[open(os.path.join(basedir, "examples/example_fewer_5.txt"), "rb")],
            name="first 10 characters again",
            description="this is an another test about the first 10 characters",
            anno_number=5,
            best_def="the best character",
...
def test_upload_same_best_worst_def(test_client, init_database):
    ...
    response = test_client.post(
        "/user/upload-project",
        data=dict(
            upload=[open(os.path.join(basedir, "examples/first_10_characters_examples.txt"), "rb")],
            name="first 10 characters again",
            description="this is an another test about the first 10 characters",
            anno_number=5,
            best_def="the best character",
            worst_def="the best character",
        ),
        follow_redirects=True,
    )
    # 1.
...
def test_upload_no_anno_number(test_client, init_database):
    ...
    response = test_client.post(
        ...
        data=dict(
            upload=[open(os.path.join(basedir, "examples/first_10_characters_examples.txt"), "rb")],
            name="first 10 characters",
            description="this is a test about the first 10 characters",
            anno_number=None,
            best_def="the best character",
            worst_def="the worst character",
        ),
