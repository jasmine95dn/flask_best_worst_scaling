...
def test_batch(test_client, init_database):
    ...
    """
    test_client.post("/annotator", data=dict(keyword="ax7832ljf", name="jung"), follow_redirects=True)
...
def test_submit_invalid_batch(test_client, init_database):
	...
	THEN check if:
		...
		3. the error messages for each invalid fields in batch pops up
	"""
    response = test_client.post("/annotator/test/batch-1", data={"action": "submit"}, follow_redirects=True)
